  ...
  try {
    ...
    const post = db.collection("posts").findOne(query, async (err, result) => {
      ...
      if (result) {
        ...
        const removeUserFromPost = {
          $pull: { joining: { _id: currentUser._id } },
        };
        // Find the post and remove the current user from the activity
        const resultPostUpdate = await db
          .collection("posts")
          .updateOne(postQuery, removeUserFromPost);
        ...
        const removeActivityFromUser = {
          $pull: { joinedActivities: { _id: postData._id } },
        };
        ...
        const resultUserUpdate = await db
          .collection("users")
          .updateOne(userQuery, removeActivityFromUser);
        ...
        const resultUpdateNotifications = await db
          .collection("users")
          .updateOne(postCreatorQuery, updateNotifications);
        ...
        return res.status(200).json({
          status: 200,
          message: "user has withdraw from the activity",
        });
      }
      ...
      // therefore the end point will handle joining
        ...
        const resultUserUpdate = await db
          .collection("users")
          .updateOne(userQuery, addPostToUser);
        ...
        const resultUpdateNotifications = await db
          .collection("users")
          .updateOne(postCreatorQuery, resultUpdateNotifications);
        ...
        return res
          .status(200)
          .json({ status: 200, message: "user has joined the activity" });
      }
    });
  } catch (err) {
    client.close();
